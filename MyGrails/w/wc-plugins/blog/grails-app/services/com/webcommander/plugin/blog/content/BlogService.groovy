...
import com.webcommander.plugin.blog.constants.DomainConstants as BlogDomainConstants
...
class BlogService implements DisposableUtilServiceModel {
    ...
    def saveComment(Map params, Map config) {
        ...
        BlogComment blogComment = new BlogComment(params)
    ...
    @Transactional
    def saveReply(Map params, Map config) {
        BlogPost post = BlogPost.get(params.postId)
        BlogComment blogComment = new BlogComment(params)
        blogComment.post = post
        ...
        blogComment.parent.addToReplies(blogComment)
        blogComment.save(flush: true)
        if (!blogComment.hasErrors()) {
            blogComment.parent.save(flush: true)
            sendNewCommentMail(blogComment)
            return true
        }
        return false
    ...
    def reactComment (Map params) {
        ...
        BlogReaction blogReaction = BlogReaction.createCriteria().get {
            eq("blogComment", blogComment);
            eq("customer", customer);
        }

        if(blogReaction){
            blogReaction.type = blogReaction.type == BlogDomainConstants.BLOG_REACTION.LIKE ? BlogDomainConstants.BLOG_REACTION.DISLIKE : BlogDomainConstants.BLOG_REACTION.LIKE
        } else{
            blogReaction = new BlogReaction()
            blogReaction.customer = customer
            blogReaction.blogComment = blogComment
            blogReaction.blogComment.addToReactions(blogReaction)
        }

        blogReaction.save(flush: true)
        if (!blogReaction.hasErrors()) {
            return true
        }
        return false
    }
