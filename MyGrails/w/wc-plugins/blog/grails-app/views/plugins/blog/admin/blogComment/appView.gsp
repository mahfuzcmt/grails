    ...
    <table class="content">
        ...
        <g:if test="${comments}">
            <g:each in="${comments}" var="comment">
                <tr ${comment.isSpam ? "class='spam'" : ""}>
                    ...
                    <td>${comment.reactions.findAll {it.type == DomainConstants.BLOG_REACTION.LIKE}.size()}</td>
                ...
                <tr class="toggle-table-row" style="display: none">
                    <td colspan="11">
                        <g:include view="/plugins/blog/admin/blogComment/commentReplyRow.gsp" model="[comment: comment]"/>
                    </td>
                </tr>
            </g:each>
        </g:if>
        <g:else>
            <tr class="table-no-entry-row">
                <td colspan="11"><g:message code="no.blog.comment.found"/></td>
            </tr>
        </g:else>
    </table>
</div>
