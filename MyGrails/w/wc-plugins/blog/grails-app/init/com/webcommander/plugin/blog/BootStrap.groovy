...
import com.webcommander.plugin.blog.constants.NamedConstants as BlogNamedConstants
...
class BootStrap {
    ...
    def init = { servletContext ->
        ...
        HookManager.register(DomainConstants.AUTO_GENERATED_PAGES.BLOG_CATEGORY_DETAILS_PAGE + "-breadcrumb") { response ->
            //TODO
            Map default_item = [:]
            default_item.title = BlogNamedConstants.PAGE_TITLE.BLOG
            response.breadcrumbItems.add(default_item)
        ...
        HookManager.register(DomainConstants.AUTO_GENERATED_PAGES.BLOG_POST_DETAILS_PAGE + "-breadcrumb") { response ->
            ...
            if((AppUtil.request.getAttribute("post").categories) && (AppUtil.request.config.post_category_breadcrumb == 'true')){
                Map item = [:]
                item.title =  AppUtil.request.getAttribute("post").categories[0].name
                item.url = app.relativeBaseUrl() + BlogResourceTagLib.RESOURCES_PATH.BLOG_CATEGORY +"/"+ AppUtil.request.getAttribute("post").categories[0].url
                response.breadcrumbItems.add(item)
            }
            //TODO
            Map default_item = [:]
            default_item.title =  BlogNamedConstants.PAGE_TITLE.BLOG
