$(function() {
    app.initCalendarWidgetForGeneralEvent = function(container, config, eventDataList) {
        ...
        function eventSummary(events) {
            ...
            $.each(events, function(idx, event) {
                ...
                var imgPath = app.baseUrl + 'resources/general-event/' + (event.image ? 'event-' + event.id + '/images/100-' + event.image : 'default/100-default.png');
                ...
                if(config.showBookNow) {
                    var btnLabel = config.labelForBookNow != 's:book.now'? config.labelForBookNow : $.i18n.prop("book.now");
                    buttonLine.append('<span><a href="' + app.baseUrl + 'generalEvent/' + event.id + '?isRecurring=' + event.isRecurring + '" class="button book-now">' + btnLabel +
                        '</a></span>');
                }
