...
class VariationTagLib {
    ...
    def variationSelectionInProductList = { attrs, body ->
        out << body()
        if(variationService.allowed() && pageScope.variables.config.show_variation_combination.toBoolean()) {
            Product productVariation = Product.get(pageScope.variables?.product?.id)
            out << g.include( view:"/plugins/variation/site/variationSelectionInProductList.gsp", model: [productVariation: productVariation, productDataVariation: pageScope.variables.product])
        }
    }

    def variationConfigView = { attrs, body ->
        String check = pageScope.config.show_variation_combination?.toBoolean() ? " checked" : ""
        out << '<div class="form-row">'
        out << '<input type="checkbox" class="single" name="category_page.show_variation_combination" value="true" uncheck-value="false"' + check + '>'
        out << '<span>' + g.message(code: "show.variation.combination") + '</span>'
        out << '</div>'
        out << body()
    }
    ...
    def productWidgetConfig = { attrs, body ->
        ...
        out << '<input type="checkbox" class="single" name="show_variation_combination" value="true" uncheck-value="false"' + check + '>'
        out << '<span>' + g.message(code: "show.variation.combination") + '</span>'
        out << '</div>'
        out << '</div>'
    }
