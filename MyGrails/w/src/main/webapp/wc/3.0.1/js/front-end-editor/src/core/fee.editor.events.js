app.FrontEndEditor.prototype.eventBuilder = (function () {
    ...
    return {
        ...
        getSavedData: function () {
            ...
            pageBody.find('.header, .footer').find("[modified-widget]").each(function () {
                var uuid = app.FrontEndEditor.utils.getUuid($(this));
                var wiElement = _self.layout.widget.get($(this));
                var css = _self.cachedWidgetData[uuid].serialized['css'];
                modified[uuid] = {uuid: uuid, type: wiElement.type, cache: wiElement.elm.data("data-cache"), css: css.toString(), js: wiElement.js, groupId: wiElement.group}
            });

            var containerCss = _self.css ? _self.css.toString() : '';
            var removed = [];
            _self.removedWidgets.every(function (index, removedWidget) {
            ...
            var _widget = pageBody.find("[modified-widget]")[0];
            if(_self.common.isInLayout(_widget)){
                data.containerId = _self.layout_id;
                data.containerType = 'layout';
                data.bodyContent = undefined;
                data.containerCss = undefined;
            }
