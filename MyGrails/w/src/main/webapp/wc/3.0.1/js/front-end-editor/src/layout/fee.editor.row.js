    ...
    var actionEvents = {
        ...
        copy: function () {
            ...
            cloneParentElement.find('.widget').each(function () {
                ...
                var newUuid = bm.getUUID();
                var prevWidgetId = $(this).attr('widget-id');
                app.FrontEndEditor.utils.copyCss(newUuid, currentWidget);
                $(this).attr('id', 'wi-' + newUuid).removeAttr('widget-id').removeAttr('cached-widget');
                _self.cachedWidgets[newUuid] = $(this);
                $(this).attr('new-widget', true);

                var _serializedData;
                var _html = currentWidget[0].outerHTML;

                if(_self.cachedWidgetData[prevUuid]) {
                    _serializedData = _self.cachedWidgetData[prevUuid].serialized;
                    _serializedData.uuid = newUuid;
                    _self.cachedWidgetData[newUuid] = _self.cachedWidgetData[prevUuid];
                    _self.cachedWidgetData[newUuid]['html'] = _html;
                    _self.cachedWidgetData[newUuid]['serialized'] = _serializedData;
                    currentWidget.data('data-cache', JSON.stringify(_serializedData));
                }else{
                    var ajaxUrl = app.baseUrl + 'widget/copyFrontEndWidget';
                    bm.ajax({
                        url: ajaxUrl,
                        data: {
                        ...
                        success: function (response) {
                            if (response.status == 'success') {
                                _serializedData = JSON.parse(response['widget']);
                                _serializedData.uuid = newUuid;
                                _self.cachedWidgetData[newUuid] = "";
                                _self.cachedWidgetData[newUuid]['html'] = _html;
                                _self.cachedWidgetData[newUuid]['serialized'] = _serializedData;
                                currentWidget.data('data-cache', JSON.stringify(_serializedData));
