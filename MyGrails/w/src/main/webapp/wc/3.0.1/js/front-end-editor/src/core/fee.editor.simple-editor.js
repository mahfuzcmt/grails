app.FrontEndEditor.prototype.simpleEditor = function () {
                            ...
                            if (!highlightedElement.parent().hasClass("redactor-container")) {
                                ...
                                if (!hasRedactor) {
                                    ...
                                    highlightedElement.parent().redactor({
                                        buttons: ['undo', 'redo', 'format', 'bold', 'italic', 'underline', 'ol', 'ul',
                                            'image', 'file', 'link'],
                                        focus: true,
                                        air: true,
                                        plugins: ['fontcolor', 'fontsize', 'alignment']
                                    });
                                }
                            }
                        }
        ...
        destroy: function () {
            if(parentElement)
            {
                if(parentElement.find(".redactor-container").length > 0){
                    parentElement.find(".redactor-container").redactor('destroy');
    ...
    var postInit = {
        initVars: function () {
            ...
            editableElements = "h1, h2, h3, h4, h5, h6, p, img, ul, *[data-style-type='text'], div[data-cloneable=true], a, blockquote, pre, li";
        ...
        selectEvent: function () {
            ...
            parentElement.on('mouseover', function (ev) {
                ...
                if (
                    ...
                    && !targetEl.hasClass("redactor-air") && targetEl.parents(".redactor-air").length < 1
                )
                {
                ...
                var targetEl = $(ev.target);
                if(targetEl.closest(".fee-highlighted-element").length===0 && targetEl.closest(".redactor-air").length===0){
                    redactorEvent.destroy();
                    _self.elementActionPanel().destroy();
                }
            });
        ...
        saveContent: function (ev) {
            ...
            var _widget = currentWidget[0];
            if(_self.common.isInLayout(_widget)){
                defaultParams.containerId = _self.layout_id;
                defaultParams.containerType = 'layout';
            }
    ...
    return {
        ...
        reset: function () {
            if(parentElement)
            {
                ...
                var domElement = parentElement[0];
                if(_self.common.isInLayout(domElement)){
                    animator = parentElement.closest('.widget-container').find('.fee-widget-animator');
                }
