<%@ page import="com.webcommander.AppResourceTagLib; com.webcommander.manager.PathManager; com.webcommander.constants.NamedConstants; com.webcommander.util.AppUtil" %>
<g:each in="${fields.sort {a, b -> a.id <=> b.id}}" var="field">
    <div class="form-row ${field.validation?.contains("required") ? 'mandatory' : ''} ${(field.htmlType == NamedConstants.PAYMENT_GATEWAY_META_FIELD_TYPE.SELECT) ? 'chosen-wrapper' : ''}  ${field.clazz}">
        ...
        <g:if test="${field.htmlType == NamedConstants.PAYMENT_GATEWAY_META_FIELD_TYPE.FILE}">
            <g:set var="physicalPath" value="${appResource.getResourcePhysicalPath(extension: appResource.getPaymentGatewayCardRelativePath(card: field.fieldFor))}"/>
            <g:if test="${field.extraAttrs == "file-type='image'" && new File("${physicalPath}").exists()}">
                <div class="custom-logo"><img src="${appResource.getPaymentGatewayCardLogoPath(cardType : field.fieldFor)}?_cc=${new Date().time}"></div>
            </g:if>
