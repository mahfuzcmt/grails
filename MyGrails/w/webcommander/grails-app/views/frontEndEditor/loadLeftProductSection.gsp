<div class="fee-body">
    <table class="fee-table left-table">
        <colgroup>
            <col style="width: 70%">
            <col style="width: 30%">
        </colgroup>
        <tr class="fee-ignore">
            <th><g:message code="product.name"/></th>
            <th class="actions-column check-all"><input class="multiple" type="checkbox"></th>
        </tr>
        <g:if test="${allProducts.size() > 0}">
            <g:each in="${allProducts}" var="product">
                <tr class="item">
                    <td>${product.name.encodeAsBMHTML()}</td>
                    <td class="actions-column" item="${product.id}" type="product">
                        <input type="checkbox" class="multiple" value="${product.id}">
                    </td>
                </tr>
            </g:each>
        </g:if>
        <g:else>
            <tr class="table-no-entry-row">
                <td colspan="2"><g:message code="no.product.found"/></td>
            </tr>
        </g:else>
    </table>
</div>
<div class="fee-footer">
    <paginator total="${count}" offset="${params.offset}" url="frontEndEditor/productConfig?onlyLeft=true" max="${params.max}"></paginator>
</div>
