...
<form class="confirm-step-form" action="${app.relativeBaseUrl()}shop/payment" method="post">
    ...
    <g:if test="${otherStoreCart}">
        <h3><g:message code="other.store.items"/></h3>
        <table class="table cartitem-table product-thumb-view inactive-cart">
            <thead>
                <tr>
            ...
            <tbody>
                <g:each in="${otherStoreCart.cartItemList}" status="i" var="cartItem">
                    <g:set var="object" value="${cartItem.object}"/>
                    <tr class="cart-item">
                        <td>${object.name.encodeAsBMHTML()} ${cartItem.variations ? "(" + cartItem.variations.join(", ").encodeAsBMHTML() + ")" : ""}</td>
                        <td><span>${currencySymbol}${(configs.price_enter_with_tax == "true" ? (cartItem.unitPrice + cartItem.unitTax) : cartItem.unitPrice).toCurrency().toPrice()}</span></td>
                        <td><span>${cartItem.quantity}</span></td>
                        <td><span class="currency-symbol">${currencySymbol}<span class="cart-item-display-total">${cartItem.cartPageDisplayTotal?.toCurrency()?.toPrice()}</span></span></td>
                    </tr>
                </g:each>
            </tbody>
        </table>
    ...
    <table class="cartitem-table product-thumb-view active-cart">
        <colgroup>
        ...
        <tr>
            ...
            <th class="product-name"><g:message code="product"/></th>
            ...
            <tr class="${(i % 2) == 0 ? 'odd' : 'even'} cart-item ${cartItem.oldQuantity ? "highlighted" : ""}" ${cartItem.oldQuantity ? "old-quantity='$cartItem.oldQuantity'" : ""} item-id="${cartItem.id}">
                ...
                <td class="product-name">
                    <div class="wrapper" data-label="<g:message code="product.name"/>:">
                        ...
                        <plugin:hookTag hookPoint="confirmStepProductColumn" attrs="${[cartItem: cartItem, object: object, configs: configs]}"/>
                    </div>
                </td>
                <td class="unit-price">
    ...
    <div class="panel-wrap">
        ...
        <div class="right-panel">
            <div class="check-out-total">
                ...
                <g:if test="${configs["show_shipping_tax"] == "true" && configs["show_shipping_and_handling_cost_separately"] == "true"}">
