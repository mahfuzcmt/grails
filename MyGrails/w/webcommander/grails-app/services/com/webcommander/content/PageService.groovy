...
class PageService {
    ...
    def addPageForStore(Long id, params) {
        ...
        DataBindingUtils.bindObjectToInstance(childPage, oldPagePropertyMap, [], ['headerWidgets', 'footerWidgets', 'dockableSections', 'metaTags', "createdBy", "createdById", "layoutId"], null);
        childPage.name = params.name
        childPage.title = params.title
        childPage.isActive = params.active.toBoolean(true)
        childPage.isDisposable = false
        childPage.disableGooglePageTracking = params.disableTracking.toBoolean(false)
        childPage.visibility = params.visibility
        childPage.visibleTo = params.visibility == DomainConstants.PAGE_VISIBILITY.RESTRICTED ?  params.visibleTo : null;
        childPage.url = params.url
        if(params.layoutid) {
            childPage.layout = Layout.get(params.layoutid)
        }
        if(params.store) {
            childPage.store = StoreDetail.get(params.store)
        }
        childPage.metaTags*.delete()
        childPage.metaTags = [];
        ...
        for (int i = 0; i < tag_names.size(); i++) {
            ...
            metaTag.save()
            childPage.addToMetaTags(metaTag).save()
        }
        childPage.customers = []
        childPage.customerGroups = []
        if(childPage.visibleTo == DomainConstants.PAGE_VISIBLE_TO.SELECTED) {
            if(params.customer) {
                def customerIds = params.list("customer")
                customerIds.each {  it ->
                    childPage.addToCustomers(Customer.proxy(it.toLong()))
                }
            }
            if(params.customerGroup) {
                def groupIds = params.list("customerGroup");
                groupIds.each {  it ->
                    childPage.addToCustomerGroups(CustomerGroup.proxy(it.toLong()))
                }
            }
        }

        childPage.createdBy = Operator.proxy(session.admin);
        childPage.save();
        ...
        widgetList.each {
            Widget widget = widgetService.copyWidget(it.id, childPage.id, it.containerType)
            if (page.headerWidgets.contains(it)) {
                childPage.addToHeaderWidgets(widget)
            } else if (page.footerWidgets.contains(it)) {
                childPage.addToFooterWidgets(widget)
            } else if (docksWidget.contains(it)) {
                dockableWidgetMap[it.id] = widget;
            } else {
                childPage.body = childPage.body.replaceFirst("uuid=\"" + it.uuid + "\"", "uuid=\"" + widget.uuid + "\"")
            }
        }
        page.dockableSections.each {
            ...
            childPage.addToDockableSections(dockSection);
        }

        storeService.addPageForStore(childPage, page, params)
        return !childPage.hasErrors();
    }
    ...
    private Closure getCriteriaClosure(Map params) {
        ...
        Closure closure = {
            ...
            if (params.isNotAssignedInStore) {
                isNull("store")
            }
            if (params.hasLayout) {
                isNotNull("layout")
