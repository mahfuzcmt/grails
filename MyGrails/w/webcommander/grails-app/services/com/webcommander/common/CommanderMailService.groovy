...
class CommanderMailService {
    ...
    Map getCommonMacros(Long storeId = null) {
        StoreDetail defaultStore = StoreDetail.first()
        StoreDetail storeDetail = storeId ? StoreDetail.findById(storeId) : StoreDetail.first()
        Map commonMacros = [
            store_name          : '',
            store_url           : '',
            store_logo          : '',
        ...
        commonMacros.each {
            switch (it.key.toString()) {
                case "store_logo" :
                    it.value = "<img src='${appResource.getStoreLogoURL(storeDetails: defaultStore, isDefault: true, isFullUrl: true)}'>"
                    break;
                case "store_url" :
    ...
    Map getMacrosAndTemplateByIdentifier(String identifier, Long storeId = null) {
        ...
        return [emailTemplate: emailTemplate, macros: macros, commonMacros: getCommonMacros(storeId), activeText: bodies.text, activeHtml: bodies.html]
    }
