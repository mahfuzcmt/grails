...
class PageController {
    ...
    def store() {
        def pageUrl = params.page ?: AppUtil.getConfig(DomainConstants.SITE_CONFIG_TYPES.GENERAL, "landing_page")
        StoreDetail store = storeService.getStore(params.store)
        Page page = pageService.getSitePage("${params.store}/${pageUrl}");
        if ((page == null) || (AppUtil.getConfig(DomainConstants.SITE_CONFIG_TYPES.ADMINISTRATION, "enable_multi_model") == "false")) {
            response.setStatus(404)
            forward(controller: "exception", action: "handle404")
            return;
        }
        ...
        storeService.setCurrentStoreInSession(params.store)
        render(view: "/layouts/sitepage", model: [page: page]);
    }
