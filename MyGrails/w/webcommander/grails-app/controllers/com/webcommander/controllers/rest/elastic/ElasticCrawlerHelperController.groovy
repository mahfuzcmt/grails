...
class ElasticCrawlerHelperController extends RestProcessor {
    ...
    def productList() {
        ...
        List fields = ["id", "name", "sku", "url", "model", "summary", "description", "globalTradeItemNumber", "title", "heading", "updated"]
    ...
    def categoryList() {
        ...
        List fields = ["id", "name", "sku", "url", "summary", "description", "title", "heading",  "updated"]
    ...
    def pageList() {
       ...
       List results = pages.collect {
           Map result = [id: it.id, name: it.name, title: it.title, url: it.url, updated: it.updated]
