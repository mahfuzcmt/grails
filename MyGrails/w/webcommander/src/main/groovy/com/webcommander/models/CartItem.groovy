...
class CartItem {
    ...
    Double cartUnitPrice; // single price
    ...
    private Double unitPriceAdjustAmount = 0
    private Map<String, Double> unitPriceAdjustAmountLog = [:]
    private Map<String, Object> unitPriceAdjustAmountObjectLog = [:]
    ...
    void calculateTax() {
        if(isTaxable) {
            ...
            if (object.product && object.product.tax >= 0) {
                unitTax = unitPriceAdjustAmount ? TaxCalculator.getTax((TaxCode)object.product.exitTaxCode, unitPrice) : object.product.tax
            } else {
    ...
    Double getUnitPrice() {
        unitPriceAdjustAmount = 0
        unitPriceAdjustAmountLog?.each {
            unitPriceAdjustAmount += it.value
        }
        return cartUnitPrice + unitPriceAdjustAmount
    }
    ...
    def addWithUnitPrice(String adjustFrom, Double adjustAmount, def adjustAmountObject = null){
        adjustWithUnitPrice(adjustFrom, adjustAmount, adjustAmountObject)
    }


    def removeWithUnitPrice(String adjustFrom) {
        if (unitPriceAdjustAmountLog.get(adjustFrom)) {
            unitPriceAdjustAmountLog.remove(adjustFrom)
        }
        if (unitPriceAdjustAmountObjectLog.get(adjustFrom)) {
            unitPriceAdjustAmountObjectLog.remove(adjustFrom)
        }
    }


    def adjustWithUnitPrice(String adjustFrom, Double adjustAmount, def adjustAmountObject = null){
        unitPriceAdjustAmountLog.put(adjustFrom, adjustAmount)
        if (adjustAmountObject){
            unitPriceAdjustAmountObjectLog.put(adjustFrom, adjustAmountObject)
        }
    }


    Double getAdjustWithUnitPriceLogValue(String adjustFrom){
        return unitPriceAdjustAmountLog.get(adjustFrom)
    }

    def getAdjustWithUnitPriceLogObject(String adjustFrom){
        return unitPriceAdjustAmountObjectLog.get(adjustFrom)
    }
