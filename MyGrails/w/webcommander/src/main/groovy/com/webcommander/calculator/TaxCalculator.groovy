...
class TaxCalculator {
    static TaxRule getAppliedRule(TaxProfile profile) {
        ...
        for(TaxRule rule : profile.rules) {
            if(rule.zones && !rule.zones.name.contains(DomainConstants.REST_OF_THE_WORLD)) {
    ...
    static TaxCode getEnteredWithTaxCode(){
        Map taxConfig = AppUtil.getConfig(DomainConstants.SITE_CONFIG_TYPES.TAX)
        Boolean priceWithTax = taxConfig.is_price_with_tax.toBoolean()
        if (priceWithTax && taxConfig.default_tax_code) {
            return TaxCode.findByName(taxConfig.default_tax_code.toString())
        }
        return null
    }


    static Double getActualPriceFromEnteredWithTax(Double amount) {
        TaxCode taxCode = getEnteredWithTaxCode()
        amount = amount - getExcludeTaxAmount((TaxCode) taxCode, amount)
        if (taxCode) {
            Currency currency
            try {
            ...
            String priceRounding = AppUtil.getConfig(DomainConstants.SITE_CONFIG_TYPES.TAX, "base_price_rounding")
            return priceRounding ? NumberUtil.getFormatedDouble(amount, priceRounding, currency.decimalPoints) : amount
        }
        return amount
    }

    static Double getExcludeTaxAmount(TaxCode code, Double amount) {
        if (!code) {
            return 0
        }
