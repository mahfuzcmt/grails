...
// Regular expression used to split event strings
    var eventSplitter = /\s+/, Events = AV.Events = {on: function (e, t, n) {
        var r, i, s;
        if (!t)return this;
...
(function (AV, window, document) {
    AV.util = {getX: function (e) {
        ...
        var specialMapping = {overlay: "Stickers", drawing: "Draw", textwithfont: "Text", colorsplash: "Splash", tiltshift: "Tilt Shift"};
        ...
        if (font.system) {
            ...
            var webFontConfig = {loading: function () {
            ...
            if (font.custom) {
                webFontConfig.custom = {families: [fontName], urls: [font.url]};
                ...
                webFontConfig.google = {families: [fontName]}
            }
...
AV.FlashAPI = function () {
    ...
    return function () {
        ...
        var _canUndo = false;
        var _canRedo = false;
        return{mapToFlashToolName: function (name) {
            switch (name) {
                case"rotate":
                    return"rotate90"
            }
                ...
                if (o["plugins"]) {
                    ...
                    for (var i = 0; i < chunks.length; ++i) {
                        ...
                        if (defaults && defaults.files) {
                            plugins.push({id: pl, presets: defaults.presetsFlash, files: defaults.files})
                        }
                    }
                    if (plugins.length > 0) {
...
(function (exports, window, document) {
    ...
    AV.ToolManager = function (controls) {
        ...
        var _toolExists = function (toolName) {
            var i, len, activeTools = controls.activeTools, toolAvailable = false;
            if (activeTools) {
...
(function (AV, window, document) {
    AV.AssetManager = function (isPremiumPartner, inAppPurchaseAllowed) {
        "use strict";
        var types = {EFFECT: "effect", STICKER: "sticker", IMAGEBORDER: "imageborder", PERMISSION: "permission", FONTPACK: "fontpack"};
        var _callbacks = {};
        var _assets;
        var _buildFrame = function () {
            if (!window.avpw_purchase_frame) {
                avpw$("#avpw_messaging").append(AV.template[AV.launchData.layout].inAppPurchaseFrame({src: AV.build["inAppPurchaseFrameURL"]}))
            }
        };
        ...
        var _getLocalDefaultAssets = function (type, callback) {
            _assets = [
                {needsPurchase: false, assetId: "default_effects", assetType: "effect", displayName: "Default", resourceUrl: "js/proclist_default_effects.js"},
                {needsPurchase: false, assetId: "original_effects", assetType: "effect", displayName: "Original", resourceUrl: "js/proclist_original_effects.js"},
                {needsPurchase: false, assetId: "original_stickers", assetType: "sticker", displayName: "Original", resourceUrl: "js/stickers_original_stickers.js"},
                {needsPurchase: false, assetId: "borders", assetType: "imageborder", displayName: "Default Image Borders", resourceUrl: "js/borders_original.js"}
            ];
            if (callback) {
                AV.util.nextFrame(function () {
                    for (var i = 0; i < _assets.length; i++) {
        ...
        api.showAssetPurchaseView = function (id, purchaseCallback) {
            var message = {messageName: "showAssetPurchaseView", assetId: id};
        ...
        api.authenticate = function () {
            var prepareAuthMessage = function (data) {
                var messageObject = {messageName: "getPartnerAssets", data: data};
                parseMessageObject(messageObject)
            };
            return function () {
                return AV.controlsWidgetInstance.serverMessaging.sendMessage({id: "avpw_auth_form", action: AV.build["partnerAssetURL"], method: "POST", dataType: "json", announcer: AV.build["asyncFeatherTargetAnnounce"], origin: AV.build["asyncImgrecvBase"], keyValues: {apikey: AV.launchData.apiKey, timestamp: AV.launchData.timestamp, signature: AV.launchData.signature, salt: AV.launchData.salt, encryptionmethod: AV.launchData.encryptionMethod}, callback: prepareAuthMessage})
            }
        }();
        api.types = types;
...
(function (AV, window, document) {
    AV.ServerMessaging = function (o) {
        ...
        var simpleXHRPost = function (url, method, dataObj, dataType, callback) {
            return avpw$.ajax({url: url, type: method, data: dataObj, dataType: dataType, error: function () {
                AV.errorNotify("ERROR_SAVING", [AV.build["imgrecvServer"]])
            }, success: callback})
        };
        var buildForm = function (id, action, target, method, keyValues) {
            var form = avpw$("<form></form>").attr({id: id, action: action, target: target, method: method || "POST"}).css({display: "none"});
            var formContents = document.createDocumentFragment();
            for (var key in keyValues) {
                if (keyValues.hasOwnProperty(key)) {
                    formContents.appendChild(avpw$("<input></input>").attr({name: key, value: keyValues[key], type: "hidden"})[0])
                }
            }
        ...
        var buildNewTargetFrame = function (id, announcer, callback) {
            ...
            if (targetHolder && targetHolder.length) {
            } else {
                targetHolder = avpw$('<div id="' + frameHolderId + '"></div>').css({position: "absolute", top: 0, left: 0}).appendTo("#avpw_holder")
            }
...
(function (exports, window, document) {
    ...
    AV.paintWidgetGetPopupEmbedDiv = function (imgElement) {
        ...
        if (imgElement) {
            ...
            var styleMap = {position: "relative"};
    ...
    AV.paintWidgetLauncher_Flash = function (paintImgIdElem, imgURL) {
        ...
        avpw$(imgElement2).css({display: "block", visibility: "hidden", position: "absolute"}).attr("src", imgURL ? imgURL : imgElement.src);
        function imgLoadComplete() {
            ...
            var holderBox = AV.template[AV.launchData.layout].flashCanvasBox({id: "avpw_canvas_swf_replace"});
            avpw$(embedElement).append(holderBox);
            AV.controlsWidgetInstance.initAllTools.call(AV.controlsWidgetInstance);
            var params = {initsize: dims, plugins: activeTools, action: {origUrl_: imgURL ? imgURL : imgElement.src, sessionID_: AV.usageTracker.getUUID(), referrerUrl_: window.location.href, url: AV.build["imgrecvServer"], name: "file"}};
    ...
    AV.paintWidgetLauncher_Flash_stage2 = function (imageWidth, imageHeight) {
        ...
        AV.controlsWidgetInstance.imageSizeTracker.setOrigSize(AV.launchData, {width: imageWidth, height: imageHeight}, {width: AV.paintWidgetInstance.width, height: AV.paintWidgetInstance.height});
    ...
    AV.mockLauncher = function (imgElement) {
        ...
        AV.util.nextFrame(function () {
            ...
            AV.controlsWidgetInstance.loaderPhase = 1;
            AV.paintWidgetInstance = {moduleLoaded: function (tool, callback) {
                return callback.call(this)
            }, setMode: stub, setCurrentLayerByName: stub, shutdown: stub, actions: {setCheckpoint: stub}};
            AV.controlsWidgetInstance.initWithPaintWidget(AV.paintWidgetInstance);
            AV.controlsWidgetInstance.showWaitThrobber(false);
            AV.controlsWidgetInstance.loaderPhase = 2;
            AV.fireLaunchComplete()
    ...
    AV.ControlsWidget = function (paintWidget, paintImgIdElem, activeTools, assetManager, serverMessaging) {
        ...
        var canvasThrobberConfig = {className: "avpw_canvas_spinner", lines: 12, length: 6, width: 2, radius: 6, color: "#fff", speed: .5, trail: 70};
        var eggThrobberConfig = {className: "avpw_tool_spinner", lines: 12, length: 6, width: 2, radius: 6, color: "#fff", speed: .5, trail: 70};
        if (AV.launchData["openType"] != "mobile") {
    ...
    AV.ControlsWidget.prototype.setupScrollPanels = function () {
        ...
        var pagerOpts = {itemCount: this.activeTools.length, itemsPerPage: this.layoutNotify(AV.launchData["openType"], "getToolsPerPage"), pageWidth: pageWidth, leftArrow: avpw$("#avpw_lftArrow"), rightArrow: avpw$("#avpw_rghtArrow"), itemBuilder: function (i) {
            toolName = _this.activeTools[i];
            toolLabel = AV.util.getUserFriendlyToolName(toolName);
            toolLabel = AV.getLocalizedString(toolLabel);
            return AV.template[AV.launchData.layout].eggIcon({optionName: toolName, capOptionName: toolLabel})
        }, pageTemplate: AV.template[AV.launchData.layout].genericScrollPanel, pipTemplate: AV.template[AV.launchData.layout].scrollPanelPip, lastPageTemplate: lastPage, lastPageContents: lastPageContents, pageContainer: avpw$("#avpw_control_main_scrolling_region"), pipContainer: avpw$("#avpw_tools_pager ul"), onPageChange: function (page) {
    ...
    AV.ControlsWidget.prototype.messager = function () {
        ...
        var api = {show: function (id, needsConfirmation, persist) {
    ...
    AV.ControlsWidget.prototype.Slider = function (config) {
        ...
        var $element = avpw$(config.element).slider({range: "min", max: config.max, min: config.min, value: config.defaultValue || config.max / 2, delay: config.delay});
    ...
    AV.ControlsWidget.prototype.save = function () {
        ...
        var getAuthenticationKeys = function () {
            return{apikey: AV.launchData.apiKey, timestamp: AV.launchData.timestamp, signature: AV.launchData.signature, salt: AV.launchData.salt, encryptionmethod: AV.launchData.encryptionMethod}
        };
        ...
        var saveImageToAsyncFeatherServices = function () {
                ...
                if (d && d.JobId) {
                    ...
                    asyncPollingTimer = window.setInterval(function () {
                        if (asyncPollingReady) {
                            AV.controlsWidgetInstance.serverMessaging.sendMessage({id: "avpw_save_form", action: AV.build["asyncImgrecvGetJobStatus"], method: "POST", dataType: "json", announcer: AV.build["asyncFeatherTargetAnnounce"], origin: AV.build["asyncImgrecvBase"], keyValues: AV.util.extend(getAuthenticationKeys(), {jobid: jobId}), callback: getJobStatusCallback})
                        }
            ...
            AV.controlsWidgetInstance.serverMessaging.sendMessage({id: "avpw_save_form", action: AV.build["asyncImgrecvCreateJob"], method: "POST", dataType: "json", announcer: AV.build["asyncFeatherTargetAnnounce"], origin: AV.build["asyncImgrecvBase"], keyValues: createJobRequestData, callback: createJobCallback})
        };
...
(function (exports, window, document) {
    ...
    AV.build = AV.build || {version: "", imgrecvServer: "imgrecvserver", flashGatewayServer: "", imgrecvBase: "", inAppPurchaseFrameURL: "", imgtrackServer: "imgtrackserver", filterServer: "", jsonp_imgserver: "", featherTargetAnnounce: "feather_target_announce_v3.html", proxyServer: "", feather_baseURL: "", feather_stickerURL: "", googleTracker: "", MINIMUM_FLASH_PLAYER_VERSION: "10.2.0"};
    ...
    baseConfig.initTool = "";
    baseConfig.cropPresets = ["Original", "Custom", ["Square", "1:1"], "3:2", "3:5", "4:3", "4:5", "4:6", "5:7", "8:10", "16:9"];
    ...
    baseConfig.giveFeedbackURL = "http://www.aviary.com/feature-requests";
        ...
        if (w["Feather_Theme"] || w["Feather_OpenType"] || w["Feather_APIKey"]) {
            AV.util.nextFrame(function () {
                ...
                w.aviaryeditor = function (imageID, imageURL, postData, inlineSite) {
                    ...
                    var config = AV.util.extend(AV.launchData, {image: imageID, url: imageURL, postData: postData, appendTo: inlineSite});
    ...
    AV.injectControls = function () {
        ...
        var s = AV.template[AV.launchData.layout].controls({internalSaveBlock: internalSaveBlock, externalSaveBlock: externalSaveBlock});
    ...
    AV.Feather = function (config) {
        ...
        var AV__featherAutoInit = function () {
            ...
            if (!AV.feather_loaded && !AV.feather_loading) {
                ...
                var styleName;
                if (AV.launchData["minimumStyling"])styleName = "feather_core_"; else styleName = "feather_theme_aviary_";
        ...
        if (apiVersion === 2) {
            AV.build["feather_baseURL"] = window.location.protocol + "//feather.aviary.com/2.6.2.182/";
            AV.build["feather_baseURL_SSL"] = window.location.protocol + "//dme0ih8comzn4.cloudfront.net/2.6.2.182/";
...
(function (AV, window, document) {
    AV.support = function (w) {
        ...
        var webkit_users = {0: /Android/i, 1: /webOS/i, 2: /iPhone/i, 3: /iPod/i, 4: /BlackBerry/i, 5: /iPad/i};
        var device;
...
var FEATHER_CONFIGS = {
    ...
    onSaveButtonClicked: function () {
        var config = this;
        FEATHER_EDITOR.getImageData(function (modifiedImageData) {
            var imgData = modifiedImageData.substring(22);
            ...
            bm.ajax({
                ...
                processData: false,
                cache: false,
                url: FEATHER_EDITOR.saveUrl,
                type: "post",
                data: formdata,
                beforeSend: function (xhr) {
                    xhr.setRequestHeader("base64_encoded", "true");
                },
                success: function () {
                    FEATHER_EDITOR.close();
                    if (FEATHER_EDITOR.afterSave) {
                        FEATHER_EDITOR.afterSave(modifiedImageData)
                    }
                }
            });
        });
...
Aviary.launchEditor = function (src, saveUrl, afterSave) {
    function loadImage() {
    ...
    if (AV.feather_loaded) {
        FEATHER_EDITOR = new Aviary.Feather(FEATHER_CONFIGS)
        loadImage()
    } else {
        $(document.body).loader();
        FEATHER_EDITOR = new Aviary.Feather($.extend({onLoad: function () {
            $(document.body).loader(false)
            loadImage()
        }}, FEATHER_CONFIGS))
    }
    FEATHER_EDITOR.saveUrl = saveUrl || src;
    FEATHER_EDITOR.afterSave = afterSave;
    return false;
}