...
(function ($) {
    ...
    $.Zebra_DatePicker = function (element, options) {
        ...
        // private properties
        var view, datepicker, icon, header, daypicker, timepicker, monthpicker, yearpicker, cleardate, current_system_month, current_system_year,
            current_system_day, first_selectable_month, first_selectable_year, first_selectable_day, selected_month, selected_year, selected_hour = 0, selected_minute = 0, selected_second = 0,
            selected_day, default_day, default_month, default_year, enabled_dates, disabled_dates, shim, start_date, end_date, last_selectable_day,
        ...
        var init = function (update) {
            ...
            var auto_select_date = default_date || plugin.settings.start_date || new Date();
            selected_day = auto_select_date.getDay();
            selected_month = auto_select_date.getMonth();
            selected_year = auto_select_date.getYear();
            selected_hour = auto_select_date.getHours();
            selected_minute = auto_select_date.getMinutes();
            selected_second = auto_select_date.getSeconds();
            ...
            var html = '' +
                ...
                '<div class="dp_timepicker"><div class="dp_timepicker_hour_block"><div class="dp_caption dp_timepicker_hour_label">' + $.i18n.prop("hour") + '</div><div class="dp_timepicker_hour"><input value="0"></div>' +
                '</div><div class="dp_timepicker_minute_block"><div class="dp_caption dp_timepicker_minute_label">' + $.i18n.prop("minute") + '</div><div class="dp_timepicker_minute"><input value="0">' +
                '</div></div><div class="dp_timepicker_second_block"><div class="dp_caption dp_timepicker_second_label">' + $.i18n.prop("second") + '</div><div class="dp_timepicker_second"><input ' +
                'value="0"></div></div><table class="dp_timepicker_done"><tbody><tr><td class="dp_timepicker_done" style="width: 100%">' + $.i18n.prop("done") + '</td></tr></tbody></table></div>' +
            ...
            timepicker = $('.dp_timepicker', datepicker);
        ...
        plugin.show = function () {
            ...
            if (default_date) {
                ...
                selected_month = default_month = default_date.getMonth();
                selected_year = default_year = default_date.getFullYear();
                selected_day = default_day = default_date.getDate();
         ...
         *                              For example, if "format" is "yyyy-MM-dd" the string should look like "2011-06-01"
         *
        ...
        var check_date = function (str_date) {
            ...
            if (plugin.settings.time) {
                var time_format = plugin.settings.time;
                if (plugin.settings.time === true) {
                    time_format = "HH:mm:ss"
                }
                format += " " + time_format;
            }

            var date = Date.parseExact(str_date, format);
            return date || false;
        };
        ...
        var generate_timepicker = function () {
            ...
            timepicker.find(".dp_timepicker_hour input").slider({
                from: 0,
                to: 23,
                step: 1,
                callback: function (h) {
            ...
            }).slider("value", selected_hour)
            if (plugin.settings.show_minute === false) {
                return;
            }
            timepicker.find(".dp_timepicker_minute input").slider({
                from: 0,
                to: 59,
                step: 1,
                callback: function (m) {
            ...
            }).slider("value", selected_minute)
            if (plugin.settings.show_second === false) {
                return;
            }
            timepicker.find(".dp_timepicker_second input").slider({
                from: 0,
                to: 59,
                step: 1,
                callback: function (s) {
            ...
            }).slider("value", selected_second)
            timepicker.find(".dp_timepicker_done").click(function () {
                select_date(selected_year, selected_month)
            })
        }
        ...
        var select_date = function (year, month, day, view, cell) {
            ...
            // construct a new date object from the arguments
                default_date = new Date(year, month, day || selected_day, selected_hour, selected_minute, selected_second),

                selected_value = format(default_date);
            $element.val(selected_value).trigger("change");
            if (view) {
                // pointer to the cells in the current view
                var view_cells = (view == 'days' ? daypicker_cells : (view == 'months' ? monthpicker_cells : yearpicker_cells))
        ...
        var update_dependent = function (date) {
            ...
            if (plugin.settings.pair) {
                ...
                $.each(plugin.settings.pair, function () {
                    ...
                    if($pair.val() && date) {
                        var pairDate = new Date($pair.val());
                        if(pairDate < date) {
                            $pair.val("");
                        }
                    }
