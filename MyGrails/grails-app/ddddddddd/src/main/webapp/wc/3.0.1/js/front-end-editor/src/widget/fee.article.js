...
_ar.beforeSave = function (ajaxSettings) {
    ...
    var content = this.configPanel.find('#articleContent');
    var addPanel = this.configPanel.find(".fee-add-panel");
    if(content.length > 0 && addPanel.is(":visible")){
        if ($.trim(content.val()).length === 0) {
            bm.notify($.i18n.prop("content.must.not.be.empty"), "error");
            return false;
        }
    }
