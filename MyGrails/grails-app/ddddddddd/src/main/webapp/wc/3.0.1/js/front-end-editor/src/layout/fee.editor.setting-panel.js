app.FrontEndEditor.settingPanel = function () {
    ...
    var afterPopupLoaded = function (content, config, popupObject, reload) {
        ...
        form.on("trash-restore", function (event, type, pageId) {
            bm.notify($.i18n.prop("<p>Page has been restored successfully</p>"), "success");
            var currentUrl = window.location.href;
            currentUrl = currentUrl.split("?")[0] + "?id=" + pageId;
            window.location = currentUrl;
            setTimeout(function() {
                location.reload();
            }, 1500);
        ...
        init: function (frontEndEditorInstance) {
            ...
            if(chosen !== undefined) {
                ...
                chosen.result_select = function (evt) {
                    ...
                    if (target.is(".tool-icon")) {
                        ...
                        if (action === 'edit') {
                            if(editorInstance.common.isAnyPopupOpen()){
                                return;
                            }
                            var pageInfo = {
                                id: item.value
                            }
                            pageForm(false, pageInfo);
                            ...
                            bm.confirm($.i18n.prop("confirm.delete", 'page'), function () {
                                ...
                                bm.ajax({
                                    ...
                                    error: function (jqXHR, textStatus, errorThrown) {
                                        if(errorThrown.attachments.at3) {
                                            bm.notify($.i18n.prop("at3.prevent.as.message", 'page', errorThrown.attachments["at3"]["as"][0]), "error");
                                        }else if(errorThrown.attachments.at2){
                                            bm.notify($.i18n.prop("at3.prevent.as.message", 'page', errorThrown.attachments["at2"]["as"][0]), "error");
                                        }
                                    },
                                    success: function (response) {
                                        if (response.status == 'success') {
                                            ...
                                            bm.notify(response.message, "error");
                                        }
                                    }
                                });
                            }, true);
                        } else if (action === "setPageAsLanding"){
                            if(editorInstance.common.isAnyPopupOpen()){
                                return;
                            }
                            bm.confirm($.i18n.prop("confirm.landing.page", 'page'), function () {
                                var ajaxUrl = app.baseUrl + 'frontEndEditor/setPageAsLanding';
                                bm.ajax({
                                    url: ajaxUrl,
                                    data: {id: item.value},
                                    type: 'post',
                                    ...
                                    success: function (response) {
                                        if (response.status == 'success') {
                                            bm.notify(response.message, "success");
                                        }else if(response.status == 'error'){
                                            bm.notify(response.message, "error");
                                        }
                                    }
            ...
            if (media === 'true' && iframePreviewElement.length < 1) {
                ...
                if (editorInstance) {
                    ...
                    function pageForm(isNewPage, pageInfo) {
                                            ...
                                            bm.ajax({
                                                ...
                                                success: function (response) {
                                                    if(response){
                                                        ...
                                                        var navigationIds = response["navigationIds"];
                                                        var navigationSelector = pageForm.find("[name='linkedNavigations']");
                                                        navigationSelector.val(navigationIds).trigger("chosen:updated");
                                                        if(isNewPage === false){
                                                            pageForm.find(".title").html($.i18n.prop('edit.page'));
                                                            navigationSelector.parent(".fee-form-element").parent(".fee-form-row").remove();
                                                        }
                                                    }
