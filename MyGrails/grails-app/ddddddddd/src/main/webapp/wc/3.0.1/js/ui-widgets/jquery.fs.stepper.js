...
;(function ($, window) {
		...
		if (!$input.hasClass("stepper-input")) {
			...
			var $stepper = $input.parent(".stepper"),
				data = $.extend({
					...
					timer: null,
                    et_data: {up: {clazz: "", attr: {}}, down: {clazz: "", attr: {}}}
				}, opts);
            if(data.et_data) {
                var up = data.et_data['up'];
                var down = data.et_data['down'];
                $stepper.find(".stepper-arrow.up").addClass(up.clazz).attr(up.attr);
                $stepper.find(".stepper-arrow.down").addClass(down.clazz).attr(down.attr);
            }
	...
	function _onMouseDown(e) {
		...
		if (!data.$input.is(':disabled') && !data.$stepper.hasClass("disabled")) {
			...
			data.timer = _startTimer(data.timer, 200, function() {
	...
	function _step(data, change) {
		...
		if (data.max !== false && value > data.max) {
			value = data.max
			data.$stepper.trigger("max")
		}
		if (data.min !== false && value < data.min) {
			value = data.min;
			data.$stepper.trigger("min")
		}
		if (value !== originalValue) {
			value = _round(value, data.digits);
			data.$input.trigger("before-step-change");
			data.$input.val(value).trigger("change", ["" + value, "" + originalValue]);
		}
	}
