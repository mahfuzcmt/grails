    ...
    <g:if test="${request.page}">
        <%
            if(!request.is_fullcalendar_loaded) {
                request.js_cache.push("plugins/simplified-event-management/fullcalendar/fullcalendar.min.js")
                request.css_cache.push("plugins/simplified-event-management/fullcalendar/fullcalendar.css")
                request.is_fullcalendar_loaded = true;
            }

            if(!request.is_event_widget_calendar_script_loaded) {
                request.js_cache.push("plugins/simplified-event-management/js/widget/calendar-widget.js")
                request.js_cache.push("plugins/simplified-event-management/js/widget/event-widget.js")
                request.is_event_widget_calendar_script_loaded = true;
            }
        %>
        <script type="text/javascript">
            $(function() {
                bm.onReady(app, "initCalendarWidgetForSimplifiedEvent", function() {
                    var config = {};
                    ...
                    app.initCalendarWidgetForSimplifiedEvent($('#calendar-' + '${id}'), config, eventDataList);
                })
            })
        </script>
    </g:if>
