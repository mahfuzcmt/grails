app.widget.simplifiedEvent = function(config) {
    app.widget.simplifiedEvent._super.constructor.apply(this, arguments);
}

var _e = app.widget.simplifiedEvent.inherit(app.widget.base);

_e.init = function() {
    var _self = this;
    app.widget.simplifiedEvent._super.init.call(this);
    var container = _self.content;

    var selectionType = container.find('.selection-type').val();
    var leftPanelUrl = app.baseUrl + 'simplifiedEventAdmin/loadEventsForSelection';
    ...
    container.find(".selection-type").on("change", function() {
        ...
        if(val == "event") {
            selectionType = 'event';
            leftPanelUrl = app.baseUrl + 'simplifiedEventAdmin/loadEventsForSelection';
...
_e.afterContentChange = function(widget, cache) {
    ...
    if(widget.elm.find('.calendar').length) {
        bm.onReady(_app, "initCalendarWidgetForSimplifiedEvent", {
            ready: function() {
                var config = {};
                ...
                container.html('');
                _app.initCalendarWidgetForSimplifiedEvent(container, config, eventDataList);
            },
            not: function() {
                var head = widget.editor.iframeWindow.$("head");
                head.append("<script src='" + app.systemResourceUrl + "plugins/simplified-event-management/fullcalendar/fullcalendar.min.js' type='text/javascript'></script>");
                head.append("<script src='" + app.systemResourceUrl + "plugins/simplified-event-management/js/widget/calendar-widget.js' type='text/javascript'></script>");
                head.append("<script src='" + app.systemResourceUrl + "plugins/simplified-event-management/js/widget/event-widget.js' type='text/javascript'></script>");
                head.append('<link rel="stylesheet" type="text/css" href="' + app.systemResourceUrl + 'plugins/simplified-event-management/fullcalendar/fullcalendar.css">');
            }
        });
    }
};
