...
class SendleCommunicator {
    ...
    public static def placeOrder(AddressData receiver, Product product, String pickupDate, String description, String receiverInstruction){
        ...
        return createOrder(sender, receiver, additionalInfo)
    }

    public static def createOrder(AddressData sender, AddressData receiver, Map additionalInfo){

        Map postParams  = getOrderData(sender, receiver, additionalInfo)
    ...
    private static def getOrderData(AddressData sender, AddressData receiver, Map additionalInfo){

        if(!additionalInfo.description){
            throw new ApplicationRuntimeException("sendle.provide.product.description")
        }

        if(!additionalInfo.receiverInstruction){
