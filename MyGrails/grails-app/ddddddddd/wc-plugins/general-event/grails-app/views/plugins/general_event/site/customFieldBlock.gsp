...
<form class="order-custom-fields-form" action="${app.relativeBaseUrl()}generalEvent/holdFields" method="post" submit-type="always">
        ...
        <div class="fields-setting">
            ...
            <g:each in="${fields}" var="field">
                <div class="form-row ${(field.validation?.indexOf('required')) >= 0 ? 'mandatory' : ''} ${field.clazz.encodeAsBMHTML()}">
                    ...
                    <g:if test="${field.type == DomainConstants.EVENT_CHECKOUT_FIELD_TYPE.TEXT}">
                    ...
                    <g:elseif test="${field.type == DomainConstants.EVENT_CHECKOUT_FIELD_TYPE.LONG_TEXT}">
                    ...
                    <g:elseif test="${field.type == DomainConstants.EVENT_CHECKOUT_FIELD_TYPE.SINGLE_SELECT_RADIO || field.type == DomainConstants.EVENT_CHECKOUT_FIELD_TYPE.MULTISELECT_CHECKBOX}">
                        <g:set var="type" value="${field.type == DomainConstants.EVENT_CHECKOUT_FIELD_TYPE.SINGLE_SELECT_RADIO ? 'radio' : 'checkbox'}"/>
                    ...
                    <g:elseif test="${field.type == DomainConstants.EVENT_CHECKOUT_FIELD_TYPE.SINGLE_SELECT_DROPDOWN || field.type ==
                            DomainConstants.EVENT_CHECKOUT_FIELD_TYPE.MULTISELECT_LISTBOX}">
                        <select title="${field.title.encodeAsBMHTML()}" name="custom.ticket${i+1}.${field.name.encodeAsBMHTML()}" ${field.type == DomainConstants.EVENT_CHECKOUT_FIELD_TYPE.MULTISELECT_LISTBOX ?
