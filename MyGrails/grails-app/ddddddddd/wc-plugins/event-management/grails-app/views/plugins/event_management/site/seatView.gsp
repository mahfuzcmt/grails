            ...
            <g:each in="${0..totalColumnDiv-1}" var="j"><span class='
                <g:if test="${ !i || (i % (rowAccess + 1) == 0) || i == totalRowDiv - 1 }">
                    ...
                    <g:if test="${!i && (j % (columnAccess + 1)) && j != totalColumnDiv - 1}">
                        ...
                        pathway'>${columnAlpha ? eventManagementApp.decimalToAlphabet(number: actualNumber) : actualNumber}
                    </g:if>
                    <g:else>
                        pathway'>&nbsp;
                    </g:else>
                ...
                <g:else>
                    <g:if test="${ !j || (j % (columnAccess + 1) == 0) || j == totalColumnDiv - 1 }">
                        <g:if test="${!j && (i % (rowAccess + 1)) && i != totalRowDiv - 1}">
                            ...
                            pathway'>${rowAlpha ? eventManagementApp.decimalToAlphabet(number: actualNumber) : actualNumber}
                        </g:if>
                        <g:else>
                            pathway'>&nbsp;
                        </g:else>
