...
class BootStrap {
    ...
    Map variation_site_config = [
            enable_matrix_view  : "false",
            enable_flate_chooser: "false"
    ]
    ...
    def tenantInit = { tenant ->
        ...
        if (!SiteConfig.findByConfigKeyAndType("enable_matrix_view", DomainConstants.SITE_CONFIG_TYPES.PRODUCT)) {
            variation_site_config.each {
                new SiteConfig(type: DomainConstants.SITE_CONFIG_TYPES.PRODUCT, configKey: it.key, value: it.value).save()
            }
            AppUtil.clearConfig(DomainConstants.SITE_CONFIG_TYPES.PRODUCT)
        }
    }
