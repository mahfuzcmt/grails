...
class PaymentGatewayService {
    ...
    public boolean updateMetaFields(Map fields) {
        try {
            fields.each { f ->
                if(f.key.indexOf(".") > -1) {
                    return
        ...
        if(fields?.CRD?.customLogo) {
            def filePath = PathManager.getResourceRoot("payment-gateway/CRD")
            ...
            MockStaticResource mockResource = new MockStaticResource(relativeUrl: appResource.getCreditCardRelativePath(), resourceName: fields.CRD.customLogo.originalFilename)
            imageService.uploadImage(fields.CRD.customLogo, "", mockResource)
            PaymentGatewayMeta.findByFieldForAndName("CRD", "customLogo").value = fields.CRD.customLogo.originalFilename
        }
        return true;
    }
